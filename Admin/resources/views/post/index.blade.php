                    ...
                    <table class="table table-striped mb-0">
                        ...
                        @foreach($items as $item)
                            <tr>
                                ...
                                <td class="text-center">{{$item->user->getFullNameAttribute()}}</td>
                                <td class="text-center">
                                    <a class="btn btn-icon simple-ajax-modal"
                                       href="{{route('post.show',$item->id)}}">
                                        <i class="las la-pen text-warning font-30 "></i>
                                    </a>
                                    <a type="button" data-bs-toggle="modal" data-bs-target="#delete-item"
                                       oncontextmenu="return false" data-action="{{route('post.delete',$item->id)}}"
                                       data-id="{{$item->id}}" data-method="delete"
                                       class="btn btn-icon modal-with-zoom-anim">
